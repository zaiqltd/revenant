...
[Print this book](https://gbdev.io/pandocs/print.html "Print this book")[Git repository](https://github.com/gbdev/pandocs "Git repository")[Suggest an edit](https://github.com/gbdev/pandocs/edit/master/src/Audio_details.md "Suggest an edit")

tl;dr:

> The PPU is a bunch of state machines, and the APU is a bunch of counters.

Digital waveformAnalog waveformMisc. digital dataDAC enable flagChannel active flagDigital Sound GeneratorChannel 1NR10NR11NR12NR13NR14Channel 2NR21NR22NR23NR24Channel 3NR30NR31NR32NR33NR34Channel 4NR41NR42NR43NR44NR50NR51NR52PCM12PCM34DAC 1DAC 2DAC 3DAC 4MixerVolumeHPFHPFOutSource: Lior "LIJI32" Halphon
...
The digital value produced by the generator, which ranges between $0 and $F (0 and 15), is linearly translated by the DAC into an analog[1](https://gbdev.io/pandocs/Audio_details.html#footnote-digital_analog) value between -1 and 1 (the unit is arbitrary).

The four analog channel outputs are then fed into the mixer[2](https://gbdev.io/pandocs/Audio_details.html#footnote-vin), which selectively adds them (depending on [`NR51`](https://gbdev.io/pandocs/Audio_Registers.html#ff25--nr51-sound-panning)) into two analog outputs (Left and Right).
Thus, the analog range of those outputs is 4× that of each channel, -4 to 4.

Then, both of these two get their respective volume scaled, once from [`NR50`](https://gbdev.io/pandocs/Audio_Registers.html#ff24--nr50-master-volume--vin-panning), and once from the volume knob (if the console has one).
...
A “DIV-APU” counter is increased every time `DIV`’s bit 4 (5 in [double-speed mode](https://gbdev.io/pandocs/CGB_Registers.html#ff4d--key1spd-cgb-mode-only-prepare-speed-switch)) goes from 1 to 0, therefore at a frequency of 512 Hz (regardless of whether double-speed is active).
...
| Event | Rate | Frequency[3](https://gbdev.io/pandocs/Audio_details.html#footnote-div_apu_freq) |
| --- | --- | --- |
...
Enabling or disabling a DAC ( [see below](https://gbdev.io/pandocs/Audio_details.html#dacs)), adding or removing it using NR51, or changing the volume in NR50, will cause an audio pop.
...
Channel x’s DAC is enabled if and only if `[NRx2] & $F8 != 0`; the exception is CH3, whose DAC is directly controlled by bit 7 of NR30 instead.
...
- Turning off its DAC
- Its [length timer](https://gbdev.io/pandocs/Audio.html#length-timer) expiring
- (CH1 only) [Frequency sweep](https://gbdev.io/pandocs/Audio_Registers.html#ff10--nr10-channel-1-sweep) overflowing the frequency
...
the “sweep timer” is clocked at 128 Hz by the [DIV-APU](https://gbdev.io/pandocs/Audio_details.html#div-apu), and the “shadow register” holds the current output period.

During a [trigger event](https://gbdev.io/pandocs/Audio.html#triggering), several things occur:

- CH1 [period value](https://gbdev.io/pandocs/Audio_Registers.html#ff13--nr13-channel-1-period-low-write-only) is copied to the “shadow register”.
- The “sweep timer” is reset.
- The “enabled flag” is set if either the [sweep pace or individual step](https://gbdev.io/pandocs/Audio_Registers.html#ff10--nr10-channel-1-sweep) are non-zero, cleared otherwise.
...
optionally negating the value (depending on the [direction](https://gbdev.io/pandocs/Audio_Registers.html#ff10--nr10-channel-1-sweep)) and summing this with the frequency
...
When the “sweep timer” is [clocked](https://gbdev.io/pandocs/Audio_details.html#div-apu) if the “enabled flag” is set and the sweep pace is not zero,
...
Each pulse channel has an internal “duty step” counter, which is used to index into [the selected waveform](https://gbdev.io/pandocs/Audio_Registers.html#ff11--nr11-channel-1-length-timer--duty-cycle) (each background stripe corresponds to one “duty step”)[4](https://gbdev.io/pandocs/Audio_details.html#footnote-pulse_lut).
The “duty step” increments at the channel’s sample rate, which is 8 times [the channel’s frequency](https://gbdev.io/pandocs/Audio_Registers.html#ff13--nr13-channel-1-period-low-write-only)).
...
When first starting up a pulse channel, it will _always_ output a (digital) zero.

CH3 has an internal “sample index” counter.
The “sample index” increments at the channel’s sample rate, which is 32 times [the channel’s frequency](https://gbdev.io/pandocs/Audio_Registers.html#ff1d--nr33-channel-3-period-low-write-only)
...
CH3 does not emit samples directly, but stores every sample read into a buffer, and emits that continuously; (re)triggering the channel does _not_ clear nor refresh this buffer, so the last sample ever read will be emitted again.
This buffer _is_ cleared when turning the APU on, so CH3 will emit a “digital 0” when first powered on.
...
When CH4 is ticked (at the frequency specified via [`NR43`](https://gbdev.io/pandocs/Audio_Registers.html#ff22--nr43-channel-4-frequency--randomness)):

1. The result of LFSR0⊙LFSR1 (`1` if bit 0 and bit 1 are identical, `0` otherwise) is written to bit 15.
2. If “short mode” was selected in [`NR43`](https://gbdev.io/pandocs/Audio_Registers.html#ff22--nr43-channel-4-frequency--randomness), then bit 15 is copied to bit 7 as well.
3. Finally, the entire LFSR is shifted right, and bit 0 selects between 0 and [the chosen volume](https://gbdev.io/pandocs/Audio_Registers.html#ff21--nr42-channel-4-volume--envelope).
...
- When triggering Ch3, the first sample to play is the previous one still in the high nibble of the sample buffer, and the next sample is the second nibble from the wave table. This is because it doesn’t load the first byte on trigger like it _should_.
...
The charge factor can be calculated for any output sampling rate as 0.999958^(4194304/rate). So if you were applying high\_pass() at 44100 Hz, you’d use a charge factor of 0.996.

* * *

1. To be clear: digital values are discrete and clear-cut; conversely, the analog domain is continuous.
The former is what computers use, the latter is what the real world is made of. [↩](https://gbdev.io/pandocs/Audio_details.html#fr-digital_analog-1)

2. Actually, VIN acts as a 5th channel fed into the mixer, whose control bits are in NR50 instead of NR51.
This was omitted from the diagram for simplicity. [↩](https://gbdev.io/pandocs/Audio_details.html#fr-vin-1)

3. Indicated values are under normal operation; the frequencies will obviously differ if writing to `DIV` to increase the counter faster. [↩](https://gbdev.io/pandocs/Audio_details.html#fr-div_apu_freq-1)

4. Actually, there is not LUT, but the manipulations done to the counter’s bits are equivalent. [↩](https://gbdev.io/pandocs/Audio_details.html#fr-pulse_lut-1)